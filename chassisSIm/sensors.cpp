...
int main(int argc, char **argv) {
  ...
  auto anymal = world.addArticulatedSystem(binaryPath.getDirectory() + "\\examples\\rsc\\anymal_c\\urdf\\anymal_sensored.urdf");
...
//  depthSensor1->setMeasurementSource(raisim::Sensor::MeasurementSource::RAISIM); // uncomment this line if you want to update the sensor using Raisim (CPU)
  ...
  for (int k = 0; k < loopN; k++) {
    ...
    if (k % 100 == 0) { // Display only every 'displayInterval' iterations

      cout << "Camera Display: " << k <<endl;

      auto cam = rgbCamera1->getImageBuffer();
      ...
      int channels = 3;  // Typically 3 for RGB format

      // Create an OpenCV Mat with the buffer data
      cv::Mat image(height, width, CV_8UC3, cam.data());

      // Convert RGB to BGR as OpenCV expects BGR format by default
